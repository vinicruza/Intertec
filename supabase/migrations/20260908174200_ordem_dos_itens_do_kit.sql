-- Patricia, 08/09/2026: ao Gerar Pedido, o kit montado no orçamento virava
-- kit oficial e a composição podia voltar em outra ordem. A ordem visual
-- estava só no JSON temporário; ao materializar em `kit_items`, ela se perdia.

alter table public.kit_items
  add column if not exists sort_order integer;

with numerados as (
  select id, row_number() over (partition by kit_id order by created_at, id) as posicao
    from public.kit_items
   where sort_order is null
)
update public.kit_items ki
   set sort_order = numerados.posicao
  from numerados
 where ki.id = numerados.id;

alter table public.kit_items
  alter column sort_order set not null,
  add constraint kit_items_sort_order_check check (sort_order > 0);

create index if not exists kit_items_kit_id_sort_order_idx
  on public.kit_items (kit_id, sort_order);
...
begin
  ...
  insert into public.kit_items (tenant_id, kit_id, product_id, quantity, sort_order)
  select v_tenant_id, v_kit_id, x.product_id, x.quantity, x.ord::integer
    from jsonb_to_recordset(p_items) with ordinality as x(product_id uuid, quantity numeric, ord bigint);
...
begin
  ...
  loop
    ...
    if v_novo then
      ...
      insert into public.kit_items (tenant_id, kit_id, product_id, quantity, sort_order)
      select v_tenant_id, v_kit_id, x.product_id, x.quantity, x.ord::integer
        from jsonb_to_recordset(v_item.ad_hoc_kit_composition) with ordinality
             as x(product_id uuid, quantity numeric, ord bigint);
