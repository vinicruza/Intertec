-- ============================================================
-- Escolher quais UFs cobram DIFAL
-- Pedido da Intertech em 21/08/2026.
-- ============================================================
--
-- Até aqui "não cobra DIFAL" só existia de duas formas:
--   1. o CANAL não aplicar (Revendas, Descpro) — vale para o pedido inteiro;
--   2. a UF não ter linha em `difal_rates` — é como SP funciona hoje.
--
-- A forma 2 obriga a APAGAR a linha para desligar, e com ela a alíquota
-- pesquisada. Esta coluna separa as duas coisas: a alíquota fica registrada, e
-- cobrar ou não vira uma escolha reversível na tela de Configurações.
--
-- Default `true`: todas as UFs que hoje têm linha continuam cobrando, então
-- nenhum pedido muda de valor ao aplicar esta migração.
alter table public.difal_rates
  add column if not exists charges_difal boolean not null default true;

comment on column public.difal_rates.charges_difal is
  'UF cobra DIFAL. Desmarcada, a alíquota fica registrada mas não entra na conta.';

-- O fechamento passa a olhar a coluna nova. Mesma cascata de antes; só a
-- linha do DIFAL muda. TS e SQL andam juntos: se um mudar sem o outro, o
...
begin
  ...
  -- A UF precisa COBRAR e o pedido precisa APLICAR: as duas coisas, e são
  -- independentes. `charges_difal` desliga por estado (Configurações);
  -- `orders.applies_difal` desliga por pedido (canal Revendas/Descpro ou
  -- marcação manual no simulador).
  select case when v_order.applies_difal and r.charges_difal then r.final_rate else 0 end
    into v_difal_rate
