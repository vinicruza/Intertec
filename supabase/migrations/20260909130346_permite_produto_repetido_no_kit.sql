-- Patricia, 09/09/2026: produto repetido dentro do kit nao e duplicidade.
-- A repeticao representa a sequencia de montagem: toalha, avental, toalha,
-- avental. Por isso `kit_items` precisa aceitar o mesmo produto em linhas
-- diferentes, preservando `sort_order`.

alter table public.kit_items
  drop constraint if exists kit_items_kit_id_product_id_key;

create index if not exists kit_items_kit_product_idx
  on public.kit_items (kit_id, product_id);
...
begin
  ...
  select v_tenant_id, v_kit_id, x.product_id, x.quantity, e.ord::integer
    from jsonb_array_elements(p_items) with ordinality as e(item, ord)
    cross join lateral jsonb_to_record(e.item) as x(product_id uuid, quantity numeric);
...
begin
  ...
  loop
    ...
    if v_novo then
      ...
      insert into public.kit_items (tenant_id, kit_id, product_id, quantity, sort_order)
      select v_tenant_id, v_kit_id, x.product_id, x.quantity, e.ord::integer
        from jsonb_array_elements(v_item.ad_hoc_kit_composition) with ordinality as e(item, ord)
        cross join lateral jsonb_to_record(e.item) as x(product_id uuid, quantity numeric);
