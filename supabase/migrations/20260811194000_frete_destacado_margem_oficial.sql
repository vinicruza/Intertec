-- Frete destacado define se o frete entra ou não na margem oficial.
-- Desmarcado: frete fica registrado para expedição, mas a margem bate com a
-- rentabilidade antiga. Marcado: frete e imposto sobre frete entram na cascata.
...
begin
  ...
  v_freight_for_margin := case when v_order.freight_paid_by_customer then p_freight else 0 end;
  v_tax := v_tax_rate*v_gross;
  v_freight_tax := v_tax_rate*v_freight_for_margin;
  v_difal := v_difal_rate*v_gross;
  v_commission := p_commission_rate*v_gross;
  v_net := v_gross-v_freight_for_margin-v_freight_tax-v_tax-v_difal-v_commission;
