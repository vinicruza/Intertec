-- Intertech, 17/09/2026:
-- 1. orçamento em aberto pode ser editado por todos os perfis operacionais;
-- 2. Edmilson deixa de ser vendedor e vira tipo de venda;
-- 3. Especial é tipo de venda sem NF, portanto sem impostos/DIFAL;
-- 4. Administrador pode excluir cliente da lista ativa.

insert into public.channels (tenant_id, name, applies_difal, tax_source, default_commission_rate, freight_model)
select t.id, 'Edmilson', false, 'icsm_table'::public.tax_source, 0.025, 'manual'::public.freight_model
  from public.tenants t
 where not exists (
   select 1 from public.channels c
    where c.tenant_id = t.id and upper(btrim(c.name)) = 'EDMILSON'
 );

insert into public.channels (tenant_id, name, applies_difal, tax_source, default_commission_rate, freight_model)
select t.id, 'Especial', false, 'none'::public.tax_source, 0.025, 'manual'::public.freight_model
  from public.tenants t
 where not exists (
   select 1 from public.channels c
    where c.tenant_id = t.id and upper(btrim(c.name)) = 'ESPECIAL'
 );
...
update public.sellers
   set active = false,
       updated_at = now()
 where active
   and upper(btrim(name)) = 'EDMILSON';

drop policy if exists orders_update_admin on public.orders;
drop policy if exists orders_update_comercial on public.orders;
create policy orders_update_simulation_operacional on public.orders for update
  using (
...
create policy order_items_write_simulation_operacional on public.order_items for all
  using (
    tenant_id = public.current_tenant_id()
    and public.current_user_role() in ('admin', 'financeiro', 'comercial')
    and exists (
...
begin
  if v_role <> 'admin' then
    raise exception 'Apenas Administrador pode excluir cliente';
  end if;
...
revoke execute on function public.deactivate_customer(uuid) from public, anon;
...
begin
  ...
  select case when c.tax_source::text = 'none' then 0 else r.icms_rate + r.pis_cofins_rate end
    into v_tax_rate
    from public.icsm_rates r
    join public.channels c on c.id = v_order.channel_id and c.tenant_id = v_tenant_id
   where r.tenant_id = v_tenant_id and r.uf = v_order.uf;
  select case when v_order.applies_difal and c.tax_source::text <> 'none' then r.final_rate else 0 end,
         case when c.tax_source::text = 'none' then false else r.difal_destacado end
    into v_difal_rate, v_difal_destacado
    from public.difal_rates r
    join public.channels c on c.id = v_order.channel_id and c.tenant_id = v_tenant_id
   where r.tenant_id = v_tenant_id and r.uf = v_order.uf;
