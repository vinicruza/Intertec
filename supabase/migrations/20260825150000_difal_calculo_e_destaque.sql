-- ============================================================
-- DIFAL entra sempre na conta; a chave por UF passa a ser o DESTAQUE
-- Regra ditada pela Intertech em 25/08/2026 (áudio da cliente).
-- ============================================================
--
-- O que estava errado. Em 21/08 nasceu `difal_rates.charges_difal` como
-- "esta UF cobra DIFAL?", e desmarcá-la ZERAVA o imposto na cascata. Treze
-- estados foram desmarcados, e os pedidos deles passaram a exibir margem sem o
-- DIFAL — até 9 pontos acima da real.
--
-- A regra correta, nas palavras da cliente: o DIFAL "precisa entrar no
-- cálculo, mas não pode ser destacada". É a mesma distinção que já existe no
-- frete — destacado e não destacado:
--
--   UF sinalizada     → a Intertech já paga hoje    → sai DESTACADO na ficha
--   UF não sinalizada → a cobrança não está          → NÃO sai destacado, mas
--                       acontecendo agora, mas         continua deduzida da
--                       pode vir a qualquer hora       margem
--
-- O destaque é sobre COMO o imposto aparece, nunca sobre SE ele existe. Quem
-- provisiona o pior caso não é surpreendido quando a cobrança chega.
--
-- Efeito: os 12 estados com alíquota registrada e destaque desligado voltam a
-- deduzir DIFAL (BA 13,5%, MA 16%, AL 14,5%, DF 13%, GO 12%, PA 12%, AC 12%,
-- AP 11%, ES 10%, RJ 10%, PR 7,5%, RS 5%). SP segue em zero porque a alíquota
-- é zero — venda interna —, e não por causa de chave nenhuma.
--
-- Pedido FECHADO não muda de valor: snapshot não se reescreve (Decisão D7).
--
-- ---------- Por que ADICIONA em vez de renomear ----------
--
-- `charges_difal` continua existindo aqui de propósito. O front que está no ar
-- lê essa coluna pelo nome; renomear derrubaria a tela de Configurações e o
-- carregamento de pedidos no intervalo entre aplicar a migração e publicar a
-- versão nova. Com a coluna nova ao lado, as duas versões funcionam, e a
-- antiga sai depois do deploy — migração 20260825160000.

alter table public.difal_rates
  add column if not exists difal_destacado boolean not null default true;

-- Herda a marcação atual: quem estava marcado "cobra" é quem a Intertech já
-- paga hoje, que é exatamente quem sai destacado.
update public.difal_rates set difal_destacado = charges_difal;

comment on column public.difal_rates.difal_destacado is
  'DIFAL sai destacado na ficha para esta UF. NAO afeta o calculo: o imposto e deduzido da margem em toda UF com aliquota, destacado ou nao (Calculations.md 7.2.1).';

comment on column public.difal_rates.charges_difal is
  'OBSOLETA desde 25/08/2026 - substituida por difal_destacado. Mantida so para o front antigo nao quebrar entre a migracao e o deploy; removida na 20260825160000.';

-- O destaque vigente no fechamento fica congelado no pedido, pelo mesmo motivo
-- que as aliquotas ficam: a ficha de um pedido fechado tem de reimprimir igual
-- daqui a um ano. Nulo nos pedidos antigos - a ficha deles cai no destaque
-- vigente da UF, que e o comportamento que ja tinham.
alter table public.orders
  add column if not exists difal_destacado_snapshot boolean;

comment on column public.orders.difal_destacado_snapshot is
  'Destaque do DIFAL da UF no momento do fechamento. Somente exibicao; nao entra em nenhuma conta.';

-- A função de fechamento acompanha. TS e SQL calculam a mesma cascata, e o
-- fechamento recusa o pedido se os dois discordarem — mudar um sem o outro
-- quebraria todo fechamento, então os dois andam nesta migração.
