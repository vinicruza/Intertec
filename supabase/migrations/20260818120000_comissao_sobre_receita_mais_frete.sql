-- Base da comissão passa a ser receita + frete (Calculations.md §6.2).
--
-- Decisão do cliente em 18/08/2026, alinhando o sistema à planilha
-- Rentabilidade 2026, que já calcula `=2,5%*($F$24+$N$6)`.
--
...
-- Trocar a base da comissão só no motor em TypeScript faria o navegador mandar
-- 445,00 enquanto o banco recalculava 420,00 — e todo pedido com frete > 0
-- morreria em "totais enviados não reconciliam".
--
-- É exatamente o mesmo tropeço que o override de DIFAL já tinha dado em
-- 05/08/2026 (Calculations.md §12.1): a regra mudou num lado só.
--
-- `p_freight` é o frete INFORMADO do pedido — o mesmo valor que a tela usa como
-- base. Frete por conta do cliente zera a DEDUÇÃO do frete, mas não a base da
-- comissão: o transporte foi vendido e o vendedor comissiona sobre ele
-- (golden test T16b). Por isso a soma usa `p_freight` direto, sem olhar a flag
-- `freight_paid_by_customer`.
--
-- Pedidos JÁ FECHADOS não são tocados: os snapshots são congelados por decisão
...
begin
  ...
  -- Base da comissão = venda + frete informado (§6.2). Antes: só v_gross.
  v_commission_base := v_gross+p_freight;
